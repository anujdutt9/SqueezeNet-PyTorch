...
def convert_model(args):
    ...
    if args.output_format == "ONNX":
        ...
        onnx_model_path = os.path.join(args.dirpath_out, f"squeezenet.onnx")
        torch.onnx.export(model, x, onnx_model_path,
                          export_params=True, opset_version=13, input_names=['input'], output_names=['output'],
                          dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}})
        # Check ONNX Model
        onnx_model = onnx.load(os.path.join(args.dirpath_out, f"squeezenet.onnx"))
        onnx.checker.check_model(onnx_model)
        # Print a Human readable representation of the graph
        onnx.helper.printable_graph(onnx_model.graph)
        log.info(f"ONNX model saved at {os.path.join(args.dirpath_out, f'squeezenet.onnx')}\n")
        ...
        # Save model
        coreml_model.save(os.path.join(args.dirpath_out, f"squeezenet.mlmodel"))
        log.info(f"CoreML model saved at {os.path.join(args.dirpath_out, f'squeezenet.mlmodel')}\n")
        ...
        tf_model_path = os.path.join(args.dirpath_out, f"squeezenet")
        torch.onnx.export(model, x, os.path.join(args.dirpath_out, f"squeezenet.onnx"),
                          export_params=True, opset_version=13, input_names=['input'], output_names=['output'],
                          dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}})
        ...
        if args.output_format == "TFLite":
            # TFLite model path
            tflite_model_path = os.path.join(args.dirpath_out, f"squeezenet.tflite")
            ...
            tfjs_model_path = os.path.join(args.dirpath_out, f"squeezenet")
...
def build_parser():
    parser = ArgumentParser(prog="SqueezeNet Export")
